...
1b. Prima di effettuare qualsiasi operazione, effettua il Login\n
...
5a. Il pulsante 'Condividi' serve per, una volta inseriti tutti i campi ed effettuato il login, ad avviare l'autocondivisione
    ...
    def login(self, driver, url, session_id):
        try:
            ...
            try:
                ...
                messagebox.showerror('WARNING', 'Non son riuscito ad effettuare il login, probabilmente le credenziali non sono esatte')
                print("[LOG] Warning: Non son riuscito ad effettuare il login, probabilmente le credenziali non sono esatte")
                print("[LOG] Chiusura del WebDriver")
                driver.quit()
                pass
                ...
                if driver_process.is_running():
                    ...
                    if chrome_process:
                        ...
                        if chrome_process.is_running():
                            try:
                                tempo = randint(20, 30)
                                print("[ + ] Attesa di %s secondi..." % (tempo))
                                sleep(tempo)
                                driver.get(val)
                                title = driver.title
                                sleep(randint(2, 6))
                                ...
                                print("[LOG] Warning: In questo gruppo '%s' non sembra possibile condividere il post" % (title))
                                pass
                    else:
    ...
    def cond_without_list(self, driver, url, session_id, post):
        try:
            ...
            if elems:
                ...
                messagebox.showinfo('Warining', 'Gruppi non trovati')
...
master = tk.Tk()
master.title("ShareBOT - V2.3.5 - FebVeg")
